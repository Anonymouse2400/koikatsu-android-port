...
public class TypefaceAnimatorEx : BaseMeshEffect
{
	...
	{
		return (!Singleton<Game>.IsInstance()) ? StartCoroutine(routine) : Singleton<Game>.Instance.StartCoroutine(routine);
	}
	...
	private IEnumerator WaitPlay()
	{
		...
		animationTime = delegate
		{
			...
			case TimeMode.Time:
				return duration;
			case TimeMode.Speed:
				return (float)characterNumber / speed;
			default:
				return 1f;
			}
		};
