...
public class HTestAibu : HTestActionBase
{
	...
	public override bool Proc()
	{
		...
		if (!flags.voice.isFemale70PercentageVoicePlay && flags.gaugeFemale >= 70f && !currentAnimatorStateInfo.IsName("K_Touch") && !currentAnimatorStateInfo.IsName("K_Loop") && flags.voice.playVoices[0] != -1 && voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.breath)
		{
		...
		else if (currentAnimatorStateInfo.IsName("M_Touch"))
		{
		...
		else if (currentAnimatorStateInfo.IsName("M_Idle"))
		{
			if (!hand.IsAction())
			{
				SetMoveCameraFlag();
			...
			else if (flags.click == HFlag.ClickKind.Front_Dislikes)
			{
				hand.ForceFinish();
				DetachItem(0);
				SetPlay("Front_Dislikes");
				backIdle = 1;
			}
			else if (flags.click == HFlag.ClickKind.Back_Dislikes)
			{
				hand.ForceFinish();
				DetachItem(1);
				SetPlay("Back_Dislikes");
				backIdle = 1;
			}
			else if (flags.click == HFlag.ClickKind.anal_dislikes || flags.click == HFlag.ClickKind.massage_mune_dislikes || flags.click == HFlag.ClickKind.massage_kokan_dislikes)
			{
		...
		else if (currentAnimatorStateInfo.IsName("A_Touch"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
				if (backIdle == -1)
		...
		else if (currentAnimatorStateInfo.IsName("A_Idle"))
		{
			if (!hand.IsAction())
			{
				SetMoveCameraFlag();
		...
		else if (currentAnimatorStateInfo.IsName("S_Touch"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
				if (backIdle == -1)
		...
		else if (currentAnimatorStateInfo.IsName("S_Idle"))
		{
			if (!hand.IsAction())
			{
				SetMoveCameraFlag();
		...
		else if (currentAnimatorStateInfo.IsName("K_Touch"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
		...
		else if (currentAnimatorStateInfo.IsName("K_Loop"))
		{
			if (flags.click == HFlag.ClickKind.orgW || flags.gaugeFemale >= 100f)
			{
				hand.ForceFinish();
		...
		else if (currentAnimatorStateInfo.IsName("Front_Dislikes"))
		{
			SetMoveCameraFlag();
		...
		else if (currentAnimatorStateInfo.IsName("Back_Dislikes"))
		{
			SetMoveCameraFlag();
		...
		else if (currentAnimatorStateInfo.IsName("Orgasm_Start"))
		{
			SetMoveCameraFlag();
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
		...
		else if (currentAnimatorStateInfo.IsName("Orgasm_Loop"))
		{
			SetMoveCameraFlag();
