...
public class DynamicBone : MonoBehaviour
{
	...
	private void AppendParticles(Transform b, int parentIndex, float boneLength)
	{
		...
		for (int i = 0; i < b.childCount; i++)
		{
			...
			{
				int num = 0;
				while (num < m_Exclusions.Count)
				{
					Transform transform2 = m_Exclusions[num];
			...
			if (!flag2)
			{
				for (int j = 0; j < m_notRolls.Count; j++)
				{
					Transform transform3 = m_notRolls[j];
					if (transform3 == b.GetChild(i))
					{
						flag = true;
						flag2 = true;
						index = i;
		...
		if (flag)
		{
			for (int k = 0; k < b.GetChild(index).childCount; k++)
			{
				bool flag3 = false;
				for (int l = 0; l < m_Exclusions.Count; l++)
				{
					Transform transform4 = m_Exclusions[l];
					if (transform4 == b.GetChild(index).GetChild(k))
					{
						flag3 = true;
						break;
					}
				...
				if (!flag3)
				{
					for (int m = 0; m < m_notRolls.Count; m++)
					{
						Transform transform5 = m_notRolls[m];
						if (transform5 == b.GetChild(index).GetChild(k))
						{
							flag = true;
							flag3 = true;
							break;
				...
				if (!flag3)
				{
					AppendParticles(b.GetChild(index).GetChild(k), count, boneLength);
				}
			}
		}
