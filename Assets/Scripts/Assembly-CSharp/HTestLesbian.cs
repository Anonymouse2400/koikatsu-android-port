...
public class HTestLesbian : HTestActionBase
{
	...
	public HTestLesbian(HFlag.DeliveryMember _membar)
		: base(_membar)
	{
	}
	...
	public override bool Proc()
	{
		...
		else if (animatorStateInfo.IsName("OLoop"))
		{
			...
			else if (!flags.isDebug && !Singleton<Voice>.Instance.IsVoiceCheck(flags.transVoiceMouth[(numCtrl == 0) ? 1u : 0u]))
			{
				flag = true;
			}
			if (flag)
