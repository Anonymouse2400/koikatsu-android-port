...
namespace CustomUtility
{
	public class CustomGuideMove : CustomGuideBase, IInitializePotentialDragHandler, IPointerDownHandler, IPointerUpHandler, IEventSystemHandler
	{
		...
		{
			if (!GlobalMethod.IsCameraActionFlag(base.guideObject.ccv2))
			{
		...
		public void OnPointerDown(PointerEventData eventData)
		{
			if (!GlobalMethod.IsCameraActionFlag(base.guideObject.ccv2))
			{
				GlobalMethod.SetCameraMoveFlag(base.guideObject.ccv2, false);
		...
		{
			GlobalMethod.SetCameraMoveFlag(base.guideObject.ccv2, true);
		...
		{
			Vector3 b = camera.transform.TransformVector(_delta.x * 0.005f, _delta.y * 0.005f, 0f) * base.guideObject.speedMove;
			Vector3 up = base.transform.up;
			up.x = Mathf.Abs(up.x);
			up.y = Mathf.Abs(up.y);
			up.z = Mathf.Abs(up.z);
			return Vector3.Scale(up, b);
		}
