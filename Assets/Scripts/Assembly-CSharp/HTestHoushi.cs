...
public class HTestHoushi : HTestActionBase
{
	private int rePlay;
	...
	public HTestHoushi(HFlag.DeliveryMember _membar)
		: base(_membar)
	{
	}
	...
	public override bool Proc()
	{
		...
		if (currentAnimatorStateInfo.IsName("Idle"))
		{
			...
			if (flags.voiceWait || flags.click == HFlag.ClickKind.insert || flags.click == HFlag.ClickKind.speedup || autoStart)
			{
				bool flag = false;
				...
				if (flag)
				{
					...
					if (flags.click == HFlag.ClickKind.insert || flags.click == HFlag.ClickKind.speedup || flags.selectAnimationListInfo == null || autoStart)
					{
						SetPlay("WLoop");
						sprite.SetHoushiActionButtonActive(true, 2);
						sprite.SetHoushiActionButtonActive(true, 3);
		...
		else if (currentAnimatorStateInfo.IsName("Stop_Idle"))
		{
			bool flag2 = false;
			if (flags.isDebug && currentAnimatorStateInfo.normalizedTime > (float)flags.debugForceLoop)
			{
		...
		else if (currentAnimatorStateInfo.IsName("WLoop"))
		{
			LoopProc(false);
		}
		...
		else if (currentAnimatorStateInfo.IsName("IN_Start"))
		{
			FinishGaugeDown();
		...
		else if (currentAnimatorStateInfo.IsName("IN_Loop"))
		{
			FinishGaugeDown();
			bool flag5 = false;
			if (flags.isDebug && currentAnimatorStateInfo.normalizedTime > (float)flags.debugForceLoop)
		...
		else if (currentAnimatorStateInfo.IsName("Oral_Idle_IN"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
		...
		else if (currentAnimatorStateInfo.IsName("Oral_Idle"))
		{
			int num2 = -1;
			if (!flags.isDebug && flags.timeWaitHoushi.IsIdleTime())
			{
		...
		else if (currentAnimatorStateInfo.IsName("M_OUT_Start"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
		...
		else if (currentAnimatorStateInfo.IsName("M_OUT_Loop"))
		{
			FinishGaugeDown();
			bool flag6 = false;
			if (flags.isDebug && currentAnimatorStateInfo.normalizedTime > (float)flags.debugForceLoop)
			...
			else if (!flags.isDebug && voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.breath && currentAnimatorStateInfo.normalizedTime > 2f)
			{
				flag6 = true;
			}
			if (flag6)
		...
		else if (currentAnimatorStateInfo.IsName("OUT_A"))
		{
			AfterProc(currentAnimatorStateInfo, 0);
		}
		else if (currentAnimatorStateInfo.IsName("Drink_IN"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
				SetPlay("Drink", false);
		...
		else if (currentAnimatorStateInfo.IsName("Drink"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
				SetPlay("Drink_A");
		...
		else if (currentAnimatorStateInfo.IsName("Drink_A"))
		{
			AfterProc(currentAnimatorStateInfo, 1);
		}
		else if (currentAnimatorStateInfo.IsName("Vomit_IN"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
				SetPlay("Vomit", false);
		...
		else if (currentAnimatorStateInfo.IsName("Vomit"))
		{
			if (currentAnimatorStateInfo.normalizedTime > 1f)
			{
				SetPlay("Vomit_A");
		...
		else if (currentAnimatorStateInfo.IsName("Vomit_A"))
		{
			AfterProc(currentAnimatorStateInfo, 2);
		}
		if (enableMotions[0])
		{
	...
	private bool LoopProc(bool _loop)
	{
		...
		else
		{
			...
			if (flags.click == HFlag.ClickKind.Idle)
			{
				SetPlay("Idle");
				sprite.SetHoushiActionButtonActive(false);
				return true;
	...
	private void AfterProc(AnimatorStateInfo _asi, int _after)
	{
		...
		{
			bool flag = false;
			if (flags.isDebug && _asi.normalizedTime > (float)flags.debugForceLoop)
			{
				flag = true;
			}
			else if (!flags.isDebug && (voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.breath || (voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.voice && !Singleton<Voice>.Instance.IsVoiceCheck(flags.transVoiceMouth[0]))))
			...
			if (flag)
			{
				...
				meta.Clear();
				rePlay = 0;
				if (!nextWait && flags.selectAnimationListInfo != null && flags.selectAnimationListInfo.mode == HFlag.EMode.houshi)
				{
					flags.click = HFlag.ClickKind.insert;
				}
			}
