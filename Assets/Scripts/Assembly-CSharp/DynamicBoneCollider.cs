...
public class DynamicBoneCollider : MonoBehaviour
{
	...
	{
		float num = m_Radius * Mathf.Abs(base.transform.lossyScale.z);
		float num2 = (m_Height - m_Radius) * 0.5f;
		if (num2 <= 0f)
		{
	...
	private void OnDrawGizmosSelected()
	{
		...
		float radius = m_Radius * Mathf.Abs(base.transform.lossyScale.z);
		float num = (m_Height - m_Radius) * 0.5f;
		if (num <= 0f)
		{
